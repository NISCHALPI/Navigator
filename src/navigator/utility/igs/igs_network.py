...
class IGSNetwork:
    ...
    def __init__(self) -> None:
        ...
        self._igs_network: pd.DataFrame = pd.read_csv(self._igs_file_path, index_col=0).infer_objects()
    ...
    def get_ellipsoid(self, station: str) -> np.ndarray:
        ...
        if station not in self._igs_network.index:
            raise ValueError(f"The station {station} is not in the IGS network.")
        return self._igs_network.loc[station, ["Latitude", "Longitude", "Height"]].values.astype(np.float64)
