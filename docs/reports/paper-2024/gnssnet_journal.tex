\documentclass[10pt, journals]{IEEEtran}
...
\title{Deep Navigation: GNSSNet for Enhanced GPS/GNSS Triangulation}

\author{
    Nischal Bhattarai, \IEEEmembership{Student Member, IEEE},
    Dr. Thejas N. Bandi, \IEEEmembership{Senior Member, IEEE},
    and Jahnvi Verma, \IEEEmembership{Student Member, IEEE}
    \thanks{Add Here!}
    \thanks{Nischal Bhattarai is with the Department of Physics, The University of Alabama, Tuscaloosa, AL 35401 USA (nbhattarai@crimson.ua.edu).}
    \thanks{Dr. Thejas N. Bandi is with the Department of Physics, The University of Alabama, Tuscaloosa, AL 35401 USA (tbandi@ua.edu).}
    \thanks{Jahnvi Verma is with the Department of Physics, The University of Alabama, Tuscaloosa, AL 35401 USA (jverma@crimson.ua.edu).}
}


\markboth{Journal of \LaTeX\ Class Files, Vol. 14, No. 8, August 2015}
{Shell \MakeLowercase{\textit{et al.}}: Deep Navigation: GNSSNet for Enhanced GPS/GNSS Triangulation}
\IEEEpubid{0000--0000/00\$00.00˜\copyright˜2015 IEEE}
\maketitle

\begin{abstract}
   We introduces GNSSNet, a robust deep neural network architecture designed for GPS/GNSS navigation. 
   Unlike conventional methods relying on the Kalman Filter, GNSSNet mitigates the need 
   for precise initialization, intricate error modeling, and high-quality measurements. 
   The proposed network exhibits enhanced resilience and can be trained to autonomously learn error patterns, 
   enabling accurate prediction of the receiver's position and velocity even in scenarios
   with weak or intermittent satellite signals. We demonstrate these capabilities through experiments 
   on both simulate as well as real-world GPS/GNSS data. Fill me ...
\end{abstract}


\begin{IEEEkeywords}
   Neural nets, Kalman filtering, Navigation ,  Reliability and robustness, Tracking
\end{IEEEkeywords}
...
\IEEEPARstart{D}{eep} learning has been transformative in diverse domains like computer vision, natural language processing, 
and speech recognition, yet its untapped potential in GPS/GNSS navigation remains a compelling area
of investigation. The intricacies of GPS/GNSS navigation are rooted in the multifaceted sources of errors 
that can compromise the accuracy of navigation solutions. While conventional post-processing techniques are effective,
their real-time application is limited while RTK requires base stations and is susceptible to signal loss. Consequently,
we explore the utilization of deep learning to address GPS/GNSS errors, focusing exclusively on raw GPS dual-frequency signals.
However, we note that same architecture can be easily extended to combined GPS/GNSS signals, IMU data and other sensor data.
Our proposed GNSSNet, a variant of KalmanNet[1], departs from traditional Kalman filters by replacing kalman gain 
computations with a parameterized neural network, circumventing the need for explicit tracking of error and state covariance matrices.
Leveraging KalmanNet as a foundational architecture, GNSSNet emerges as a promising solution for filtering GPS/GNSS signals.

Continuous satellite tracking is imperative for the traditional Kalman filter to maintain tracking accuracy, making it susceptible to signal loss, 
as the state variable monitors error terms  crucial for modeling specific satellite errors like floating-point ambiguity and biases. This becomes particularly challenging in environments 
with intermittent signal visibility, such as urban canyons and tunnels. However, our proposed deep learning model adeptly overcomes these challenges by removing manual error term tracking,
which enables the aggergation of measurements to enhances overall robustness and adaptability. To achieve this, an attention mechanism is employed to aggregate measurements before 
feeding them into KalmanNet for state estimation. Consequently, a Set Transformer[2] is employed to project any size of measurements into a fixed-size latent space, ensuring adaptability in environments
with sudden signal losses.
